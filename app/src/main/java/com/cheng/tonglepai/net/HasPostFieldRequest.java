...
import com.cheng.retrofit20.http.HasPostFieldlCmd;
...
public class HasPostFieldRequest extends BaseHttpRequest<List<HasPostFieldData>> {

    private Context mContext;

    public HasPostFieldRequest(Context context) {
    ...
    private RequestParams getParams(String userId, String page) {
        ...
        parameters.putParams(HasPostFieldlCmd.K_PAGE,page);
        parameters.putParams(HasPostFieldlCmd.K_USER_ID,userId);
        parameters.putParams(HasPostFieldlCmd.K_TOKEN, HttpConfig.newInstance(mContext).getAccessToken());
        return parameters;
    }

    private HttpCommand newHttpCommand(String userId,String page) {
        HttpCommand httpCmd = new HasPostFieldlCmd(mContext, getParams(userId,page));

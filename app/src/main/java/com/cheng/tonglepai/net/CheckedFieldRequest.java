...
import com.cheng.retrofit20.http.GetCheckedFielfCmd;
...
public class CheckedFieldRequest extends BaseHttpRequest<List<HasPostFieldData>> {

    private Context mContext;

    public CheckedFieldRequest(Context context) {
    ...
    private RequestParams getParams(String userId, String page) {
        ...
        parameters.putParams(GetCheckedFielfCmd.K_PAGE,page);
        parameters.putParams(GetCheckedFielfCmd.K_USER_ID,userId);
        parameters.putParams(GetCheckedFielfCmd.K_TOKEN, HttpConfig.newInstance(mContext).getAccessToken());
        return parameters;
    }

    private HttpCommand newHttpCommand(String userId,String page) {
        HttpCommand httpCmd = new GetCheckedFielfCmd(mContext, getParams(userId,page));

...
public class InvestorSendSmsRequest extends BaseHttpRequest<TestData> {
    ...
    private RequestParams getParams(String tel) {
        ...
        parameters.putParams(InvestorSendSmsCmd.K_COUNT, tel);
        parameters.putParams(InvestorSendSmsCmd.K_USEID, HttpConfig.newInstance(mContext).getUserid());
        return parameters;
    }

    private HttpCommand newHttpCommand(String tel) {
        HttpCommand httpCmd = new InvestorSendSmsCmd(mContext, getParams(tel));

...
public class PartnerSiteIncomeAdpter extends BaseAdapter {
    private Context context;
    private List<PartnerSiteIncomeBean.DataBean> mData = new ArrayList<>();

    public PartnerSiteIncomeAdpter(Context context, List<PartnerSiteIncomeBean.DataBean> mList) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (null == convertView) {
            convertView = View.inflate(context, R.layout.view_item_partner_site_income, null);
        ...
        if(!TextUtils.isEmpty(mData.get(position).getStore_name()))  holder.tv_siteName.setText(mData.get(position).getStore_name());
         holder.tv_deviceCode.setText("设备："+mData.get(position).getDevice_nums()+"台");
